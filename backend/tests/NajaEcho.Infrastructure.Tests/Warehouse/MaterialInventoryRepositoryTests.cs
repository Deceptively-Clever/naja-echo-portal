...
public sealed class MaterialInventoryRepositoryTests : IAsyncLifetime
{
    ...
    public async Task GetMaterialsAsync_NoFilters_ReturnsAllRows()
    {
        ...
        var repo = MakeRepo();
        await repo.AddOrIncrementAsync(commodity.Id, user.Id, "Bay 1", 1m, 500, default);
    ...
    public async Task GetMaterialsAsync_DefaultSort_MaterialNameAsc_QualityDesc_OwnerNameAsc_LocationAsc()
    {
        ...
        await repo.AddOrIncrementAsync(alpha.Id, alice.Id, "Bay 1", 1m, 300, default);
        await repo.AddOrIncrementAsync(alpha.Id, alice.Id, "Bay 1", 1m, 700, default);
        // Same commodity (Alpha), same quality, different owners -> owner name asc
        await repo.AddOrIncrementAsync(alpha.Id, bob.Id, "Bay 1", 1m, 300, default);
        // Different commodity (Zeta) sorts after Alpha
        await repo.AddOrIncrementAsync(zeta.Id, alice.Id, "Bay 1", 1m, 500, default);
    ...
    public async Task GetMaterialsAsync_MaterialFilter_MatchesNameOrCodeCaseInsensitively()
    {
        ...
        var repo = MakeRepo();
        await repo.AddOrIncrementAsync(titanium.Id, user.Id, "Bay 1", 1m, 500, default);
        await repo.AddOrIncrementAsync(quantanium.Id, user.Id, "Bay 1", 1m, 500, default);
    ...
    public async Task GetMaterialsAsync_OwnerFilter_RestrictsToSingleOwner()
    {
        ...
        var repo = MakeRepo();
        await repo.AddOrIncrementAsync(commodity.Id, alice.Id, "Bay 1", 1m, 500, default);
        await repo.AddOrIncrementAsync(commodity.Id, bob.Id, "Bay 1", 1m, 500, default);
    ...
    public async Task GetMaterialsAsync_LocationFilter_RestrictsToSingleLocation()
    {
        ...
        var repo = MakeRepo();
        await repo.AddOrIncrementAsync(commodity.Id, user.Id, "Bay 1", 1m, 500, default);
        await repo.AddOrIncrementAsync(commodity.Id, user.Id, "Dock 3", 1m, 600, default);
    ...
    public async Task GetMaterialsAsync_QualityRange_IsInclusiveBetween()
    {
        ...
        var repo = MakeRepo();
        await repo.AddOrIncrementAsync(commodity.Id, user.Id, "Bay 1", 1m, 100, default);
        await repo.AddOrIncrementAsync(commodity.Id, user.Id, "Bay 1", 1m, 500, default);
        await repo.AddOrIncrementAsync(commodity.Id, user.Id, "Bay 1", 1m, 1000, default);
    ...
    public async Task GetMaterialsAsync_CombinedFilters_AppliesAndLogic()
    {
        ...
        var repo = MakeRepo();
        await repo.AddOrIncrementAsync(titanium.Id, alice.Id, "Bay 1", 1m, 500, default);
        await repo.AddOrIncrementAsync(titanium.Id, bob.Id, "Bay 1", 1m, 500, default);
        await repo.AddOrIncrementAsync(titanium.Id, alice.Id, "Dock 3", 1m, 500, default);
    ...
    public async Task AddOrIncrementAsync_NewKey_InsertsRowAndReturnsIsNewTrue()
    {
        ...
        var repo = MakeRepo();
        var (row, isNew) = await repo.AddOrIncrementAsync(commodity.Id, user.Id, "Bay 1", 1.5m, 500, default);
    ...
    public async Task AddOrIncrementAsync_SameKey_IncrementsQuantityAndReturnsIsNewFalse_NoDuplicateRow()
    {
        ...
        var repo = MakeRepo();
        await repo.AddOrIncrementAsync(commodity.Id, user.Id, "Bay 1", 1m, 500, default);
        var (row, isNew) = await repo.AddOrIncrementAsync(commodity.Id, user.Id, "Bay 1", 2m, 500, default);
    ...
    public async Task AddOrIncrementAsync_QualityIsPartOfConflictKey_NeverAlteredByUpdate()
    {
        ...
        var repo = MakeRepo();
        await repo.AddOrIncrementAsync(commodity.Id, user.Id, "Bay 1", 1m, 300, default);
        // Different quality -> distinct key, not an increment of the first row
        var (row, isNew) = await repo.AddOrIncrementAsync(commodity.Id, user.Id, "Bay 1", 1m, 700, default);
    ...
    public async Task UpdateQuantityAsync_SetsAbsoluteQuantity_LeavesOtherFieldsUnchanged()
    {
        ...
        var repo = MakeRepo();
        var (row, _) = await repo.AddOrIncrementAsync(commodity.Id, user.Id, "Bay 1", 1m, 700, default);
    ...
    public async Task UpdateQuantityAsync_RawUpdateToZeroOrNegative_RejectedByCheckConstraint()
    {
        ...
        var repo = MakeRepo();
        var (row, _) = await repo.AddOrIncrementAsync(commodity.Id, user.Id, "Bay 1", 1m, 500, default);
